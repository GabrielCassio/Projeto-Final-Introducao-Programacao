...
SKILL_HITBOX_SCALE = 0.8

def _scale_rect_center(r: pygame.Rect, scale: float) -> pygame.Rect:
    nw = max(1, int(r.width * scale))
    nh = max(1, int(r.height * scale))
...
class FireWave(pygame.sprite.Sprite):
    def __init__(self, x, y, direction_vec, speed, frames, damage, groups, hitbox_inflate=(0, 0), lifetime_px=1200, hitbox_scale=1.0):
        super().__init__()
        if groups:
            if "all" in groups:
                groups["all"].add(self)
        ...
        self.hitbox_scale = float(hitbox_scale)
        ...
        if self.hitbox_scale != 1.0:
            self.hitbox = _scale_rect_center(self.hitbox, self.hitbox_scale)
...
class DemonBoss(pygame.sprite.Sprite):
    def __init__(self, pos_midbottom, player, groups, screen_rect):
        ...
        self.skill_hitbox_scale = SKILL_HITBOX_SCALE
    ...
    def _start_jump(self):
        ...
        for _ in range(j["count"]):
            ...
            s = self.skill_hitbox_scale
            area = pygame.Rect(0, 0, int(j["w"] * s), int(j["h"] * s))
            area.center = (rx, ry)
    ...
    def _perform_land_impact(self):
        ...
        ShockwavePush(
            self.rect.center,
            int(sw["max_radius"] * self.skill_hitbox_scale),
            self.player,
            self.groups_ref,
        ...
        for dx, dy in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
            ...
            FireWave(
                ...
                hitbox_scale=self.skill_hitbox_scale,
            )

        imp = j["impact"]
        dist = pygame.Vector2(self.player.rect.center).distance_to(self.rect.center)
        if dist < (imp["radius"] * self.skill_hitbox_scale):
        ...
        if self.explosion_trigger_time > 0 and now >= self.explosion_trigger_time:
            ...
            for pos in self.pending_explosions:
                CircleExplosion(
                    ...
                    jexp["damage"],
                    self.groups_ref,
                    hitbox_size=(int(jexp["w"] * self.skill_hitbox_scale), int(jexp["h"] * self.skill_hitbox_scale)),
                )
        ...
        if self.state == "chase":
            ...
            if dist < cfg["range"]:
                ...
                if dist > 0:
                    ...
                    stop_dist = DEMON_STATS.get("chase_stop_distance", 70)
                    step = min(self.speed, max(0.0, dist - stop_dist))
                    if step > 0:
                        self.rect.center += direction * step
            ...
            if diff.length() > 0:
                ...
                dist = diff.length()
                stop_dist = DEMON_STATS.get("chase_stop_distance", 70)
                step = min(self.speed * 0.5, max(0.0, dist - stop_dist))
                if step > 0:
                    self.rect.center += direction * step
