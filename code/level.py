...
BASE_DAMAGE = 20
CRIT_DAMAGE = 35
CRIT_CHANCE_DEFAULT = 0.20

SHAKE_ON_PLAYER_HURT_INTENSITY = 1
SHAKE_ON_PLAYER_HURT_DURATION = 3

SHAKE_ON_CRIT_INTENSITY = 3
SHAKE_ON_CRIT_DURATION = 6
...
class Level:
    ...
    def _roll_crit(self, attack_sprite):
        if isinstance(attack_sprite, Cracha):
            return False
        chance = getattr(self.player, "crit_chance", CRIT_CHANCE_DEFAULT)
        try:
            chance = float(chance)
        except Exception:
            chance = CRIT_CHANCE_DEFAULT
        return random.random() < chance

    def _spawn_damage_text(self, target_sprite, amount, is_crit):
        color = 'red' if is_crit else 'yellow'
        size = 12 if is_crit else 14
        FloatingText(
            (target_sprite.rect.centerx, target_sprite.rect.top - 12),
            str(int(amount)),
    ...
    def trigger_enemy_attack(self, owner, type, pos, direction, damage):
        if type == 'projectile':
            ...
            life = getattr(owner, "projectile_life_time", 3000)
    ...
    def player_attack_logic(self):
        ...
        for attack_sprite in self.attack_sprites:
            if not hasattr(attack_sprite, "hit_list"):
                attack_sprite.hit_list = set()

            collision_sprites = pygame.sprite.spritecollide(attack_sprite, self.attackable_sprites, False)

            for target_sprite in collision_sprites:
                if target_sprite in attack_sprite.hit_list:
                    continue
                attack_sprite.hit_list.add(target_sprite)

                is_crit = self._roll_crit(attack_sprite)
                dmg = CRIT_DAMAGE if is_crit else BASE_DAMAGE

                setattr(attack_sprite, "damage", dmg)
                setattr(attack_sprite, "crit", is_crit)
                setattr(attack_sprite, "is_crit", is_crit)
                ...
                if hasattr(target_sprite, 'get_damage'):
                    ...
                    try:
                        target_sprite.take_damage(dmg, source_pos=self.player.rect.center, crit=is_crit)
                    except TypeError:
                        target_sprite.take_damage(dmg)

                self._spawn_damage_text(target_sprite, dmg, is_crit)

                if is_crit:
                    self.visible_sprites.add_shake(SHAKE_ON_CRIT_INTENSITY, SHAKE_ON_CRIT_DURATION)
    ...
    def damage_player(self, amount, attack_type):
        if self.player.vulnerable:
            ...
            self.trigger_particles(self.player.rect.center, amount=6, color='white')
            self.visible_sprites.add_shake(SHAKE_ON_PLAYER_HURT_INTENSITY, SHAKE_ON_PLAYER_HURT_DURATION)
