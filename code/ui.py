...
def _asset_path(*parts) -> str:
    base_dir = os.path.dirname(__file__)  # ui.py está em /code
    return os.path.abspath(os.path.join(base_dir, "..", *parts))  # ../assets/...
...
def _load_spritesheet_frames_cached(rel_path: tuple, frame_size=None, scale_to=None) -> list[pygame.Surface]:
    ...
    # Inferência padrão: frames quadrados (muito comum)
...
class UI:
    def __init__(self):
        ...
        # --- ÍCONE GRANDE (CRAChÁ quando tiver) ---
        self.box_size = 90
    ...
    def show_counter(self, amount, offset_y, icon_surface=None, icon_color='gold'):
        # (Seu show_counter atual desenha círculo e brilho pra alma) :contentReference[oaicite:1]{index=1}
        ...
        # --- NOVO: se tiver imagem, usa ela; senão, cai no círculo antigo ---
    ...
    def display(self, player):
        ...
        # 2. ÍCONE GRANDE: quando tiver crachá, mostra o crachá (não a arma antiga) :contentReference[oaicite:2]{index=2}
        ...
        # 3. Contadores (Moedas embaixo, Almas em cima) :contentReference[oaicite:3]{index=3}

        # moedas (sem asset -> mantém círculo)
        self.show_counter(player.coins, 0, icon_surface=None, icon_color='gold')

        # souls (com sprite sheet -> anima)
        soul_icon = None
        if self.soul_frames:
